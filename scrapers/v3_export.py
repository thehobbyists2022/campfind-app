...
def main():
    ...
    # v39 Galileo venue-address fix (from orphaned v37 patch + zip evidence):
    # 12 wrong-state records re-stated (extend v37 fix) + 51 official addresses.
    v39 = {}
    v39_path = os.path.join(ROOT, "app", "aca_camps_v39.json")
    if os.path.exists(v39_path):
        v39 = json.load(open(v39_path, encoding="utf-8"))
    # v40 more thin-state real camps — AK Camp Fire 3 + ME Chewonki + WV Alleghany
    # + MS Gulfport Summer Day Camp 4 sites (official pages). (optional)
    v40 = []
    v40_path = os.path.join(ROOT, "app", "aca_camps_brands_v40.json")
    if os.path.exists(v40_path):
        v40 = json.load(open(v40_path, encoding="utf-8"))["camps"]
    # v41 YMCA camps for thin states — Sioux Falls SD 6 + Bangor ME 4 + Casper WY 1.
    v41 = []
    ...
    # v43 Snapology locations — 73 new US locations (extends v13, 129 → 202).
    v43 = []
    v43_path = os.path.join(ROOT, "app", "aca_camps_brands_v43.json")
    if os.path.exists(v43_path):
        v43 = json.load(open(v43_path, encoding="utf-8"))["camps"]
    # v44 ACA accredited camps — 138 selected from full find.acacamps.org crawl
    # (5,427 valid profiles; deduped + balanced to hit exactly 5,000 total).
    v44 = []
    v44_path = os.path.join(ROOT, "app", "aca_camps_brands_v44.json")
    if os.path.exists(v44_path):
        v44 = json.load(open(v44_path, encoding="utf-8"))["camps"]
    ...
    # v39 Galileo re-state: extend the v37 id-state fix to Galileo records whose
    # state+coords were BOTH wrong (zip evidence in official venue addresses).
    v39_renames = v39.get("renames", {})
    v39_renamed = 0
    for c in merged:
        if c["id"] in v39_renames:
            c["id"] = v39_renames[c["id"]]
            v39_renamed += 1
    ...
    # v39 Galileo venue addresses: patch existing records in place (address +
    # precise coords + state where the record was re-stated). Applied AFTER
    # renames so keys are the current ids.
    v39_patched = 0
    for c in merged:
        if c["id"] in v39.get("updates", {}):
            for k, v in v39["updates"][c["id"]].items():
                c[k] = v
            v39_patched += 1

    # v40 more thin-state camps: add (unique ids).
    for c in v40:
        if c["id"] in seen_ids:
            dup_skipped += 1
            continue
    ...
    # v43 Snapology locations: add (unique ids).
    for c in v43:
        if c["id"] in seen_ids:
            dup_skipped += 1
            continue
    ...
    # v44 ACA accredited camps: add (unique ids).
    for c in v44:
        if c["id"] in seen_ids:
            dup_skipped += 1
            continue
    ...
    print(f"merged total: {len(merged)} (v2 {len(v2)} + v3 {len(v3)} + v4 {len(v4)} + v5 {len(v5)} + v6 {len(v6)} + v7 {len(v7)} + v8 {len(v8)} + v9 {len(v9)} + v12 {len(v12)} + v13 {len(v13)} + v14 {len(v14)} + v15 {len(v15)} + v16 {len(v16)} + v17 {len(v17)} + v18 {len(v18)} + v19 {len(v19)} + v20 {len(v20)} + v21 {len(v21)} + v22 {len(v22)} + v23 {len(v23)} + v24 {len(v24)} + v25 {len(v25)} + v26 {len(v26)} + v27 {len(v27)} + v28 {len(v28)} + v29 {len(v29)} + v30 {len(v30)} + v31 {len(v31)} + v32 {len(v32)} + v33 {len(v33)} + v34 {len(v34)} + v35 {len(v35)} + v36extra {len(v36_extra)} + v38 {len(v38)} + v40 {len(v40)} + v41 {len(v41)} + v42 {len(v42)} + v43 {len(v43)} + v44 {len(v44)}, dup skipped {dup_skipped}, v36 patched {patched}, v37 renamed {renamed}, v39 renamed {v39_renamed}, v39 patched {v39_patched})")
